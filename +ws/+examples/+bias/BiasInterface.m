...
classdef biasInterface < handle
    ...
    properties
        address = '';
        port = [];
        %biasExeDir = 'bias_fc2_win64-v0.53-plugin-dev-07';
        biasExeDir = 'C:/Users/labadmin/Desktop/bias_fc2_win64-v0.54-plugin-dev-08' ;
        %biasExeFile = 'bias_gui_v0p53.exe';%'bias_gui_v049.exe';
        biasExeFile = 'bias_gui_v0p54.exe' ;
    end
    ...
    methods
        function self = biasInterface(address, port)
            ...
            numInastanceBIAS = length(strfind(cmdout,self.biasExeFile));
            if  numInastanceBIAS >= 2
                fprintf(['More than one instance of ' self.biasExeFile 'found running!\n']);
                fprintf('Quitting all instances and restarting BIAS.\n');
                system(['taskkill /F /IM ' self.biasExeFile]);
                system(['start "Matlab driven BIAS" ' self.biasExeDir '\' self.biasExeFile']);
            elseif numInastanceBIAS == 0
                fprintf('Launching BIAS...');
                path = which(self.biasExeFile);
                if ~isempty(path)
                    system(['start "Matlab driven BIAS" ' path]);
                    fprintf('done.\n');
                else
                    fprintf('Executable "%s" not found.\n', path);
                end
            elseif numInastanceBIAS == 1
                %Note: Do nothing singleton
            end
        end
        
        function kill(self)
            fprintf(['One instance of ' self.biasExeFile ' found running!\n']);
            system(['taskkill /F /IM ' self.biasExeFile]);
        end
