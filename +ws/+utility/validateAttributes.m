...
args = ws.utility.filterPVArgs(varargin,VALIDPROPS);
props = args(1:2:end);
vals = args(2:2:end);
...
if s.AllowEmptyDouble && ws.utility.isEmptyDouble(val) 
    return;
end
...
if iscellstr(options)
    assert(ws.utility.isString(val) && ismember(val,options),EID,EMSG);
    cls = 'char';
