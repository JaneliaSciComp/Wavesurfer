function Voltage_Continuous_Output_NoRegeneration()
%% Continuous Voltage Output
% This examples demonstrates a continuous buffered analog output using the ws.dabs.ni.daqmx adapter
...
try
    ...
    hTask.registerEveryNSamplesEvent(@callbackFunction,numberOfSamplesToWrite);
    hTask.start();
...
function callbackFunction(src,evt)
    hTask = src;
    if ~isempty(evt.errorMessage)
        delete(hTask);
        error(evt.errorMessage);
    else
        numberOfSamplesToWrite = 1000;
        outputData = linspace(0,10,numberOfSamplesToWrite);
        hTask.writeAnalogData(outputData, 5);
        disp('Refreshed output data');
    end
