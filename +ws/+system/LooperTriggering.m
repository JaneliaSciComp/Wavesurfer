...
%         end
        ...
        function err = configureForRun(self,wavesurferTriggeringSubsystem)
            % Configure self for a run, copying settings from
            % wavesurferModel
            
            
            self.IsSweepBased = wavesurferTriggeringSubsystem.IsSweepBased ;
            self.NSweepsPerRun = wavesurferTriggeringSubsystem.NSweepsPerRun ;
            self.Triggering.configureForRun(wavesurferTriggeringSubsystem.Triggering) ;  % Do this first so other subsystems have trigger schemes to point to
            self.Acquisition.configureForRun(wavesurferTriggeringSubsystem.Acquisition) ;
            self.Stimulation.configureForRun(wavesurferTriggeringSubsystem.Stimulation) ;
            err = [] ;
        end
